...
def process_batch():
    ...
    if valid_records:
        ...
        with conn.cursor() as cursor:
            ...
            try:
                cursor.execute("""
                    CREATE TEMP TABLE breadcrumb_staging(
                        tstamp TIMESTAMP,
                        latitude FLOAT,
                        longitude FLOAT,
                        speed FLOAT,
                ...
                output.seek(0)
                cursor.copy_from(output, 'breadcrumb_staging', sep=',',
                                 columns=('tstamp', 'latitude', 'longitude', 'speed', 'trip_id'))

                cursor.execute("""
                    INSERT INTO breadcrumb (tstamp, latitude, longitude, speed, trip_id)
                    SELECT bs.tstamp, bs.latitude, bs.longitude, bs.speed, bs.trip_id
                    FROM breadcrumb_staging bs
                    WHERE EXISTS (
                        SELECT 1
                        FROM trip t
                        WHERE  t.trip_id = bs.trip_id
                    );
                """)
