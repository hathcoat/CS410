...
def load(conn, icmdlist):
    ...
    with conn.cursor() as cursor:
        ...
        #Commented these 2 lines out to test copy_from
#        for cmd in icmdlist:
#            cursor.execute(cmd)
...
#        """)
        ...
        with open(Datafile, 'r') as f:
            ...
            for row in reader:
                ...
                for field in row:
                    if field == "":
                        cleaned_row.append('\\N')  # Postgres NULL marker
                    else:
                        cleaned_row.append(field)
                output.write(','.join(cleaned_row) + '\n')

            output.seek(0)
            cursor.copy_from(output, 'censusdata', sep=',')
         #   cursor.execute("SELECT COUNT(*) FROM censusdata;")
